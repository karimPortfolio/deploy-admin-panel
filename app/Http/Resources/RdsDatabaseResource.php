...
class RdsDatabaseResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'engine' => $this->whenHas('engine', fn($engine) => $engine->toArray()),
            'db_name' => $this->whenHas('db_name'),
            'allocated_storage' => $this->whenHas('allocated_storage', fn($value) => "$value GB"),
            'storage_type' => $this->whenHas('storage_type', fn($storage) => $storage->toArray()),
            'db_instance_class' => $this->whenHas('db_instance_class', fn($class) => $class->toArray()),
            ...
            'backup_retention_period' => $this->whenHas('backup_retention_period', fn ($value) => $value.' '.__('messages.days')),
            ...
            'snapshots' => RdsDatabaseSnapshotResource::collection($this->whenLoaded('snapshots')),

...
const { data, fetch, loading, options, onRequest } =
    useResourceIndex("admin/servers");

const { destroy, destroying, destroyed } = useResourceDestroy("admin/servers");

const { update: startServer, updating } = useResourceUpdate(
    () => `admin/servers/${itemToChangeStatus.value.id}/start`
);

const { update: stopServer, updating: stopping } = useResourceUpdate(
    () => `admin/servers/${itemToChangeStatus.value.id}/stop`
);
